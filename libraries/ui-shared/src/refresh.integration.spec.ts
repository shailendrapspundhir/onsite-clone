...
// ensure fetch exists in Jest/node environment
...
describe('Auth refresh integration', () => {
  test('register -> login -> refreshTokens returns new tokens', async () => {
    ...
    try {
      ...
      // service not up; skip integration test
      return;
    }
    ...
    const registerResp = await fetch(AUTH_URL, {
      ...
      body: JSON.stringify({
        ...
        variables: { input: { email, password, userType: 'WORKER', firstName: 'Test', lastName: 'User' } },
      }),
    });
